...
public class MavenUtil {
    ...
    static MavenSettings parseMavenSettings(File settingsFile) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < mirrorList.getLength(); i++) {
                ...
                String mirrorOf =
                        mirror.getElementsByTagName("mirrorOf").item(0).getTextContent().trim();
            ...
            for (int i = 0; i < proxyList.getLength(); i++) {
                ...
                if (host != null) {
                    ...
                    if (port != null)
                        settings.mProxyPort = Integer.parseInt(port.getTextContent().trim());
                    break;
                }
            }
        } catch (IOException e) {
    ...
    static String parseLastReleaseBuild(File mdFile, ProtocVersion protocVersion)
            throws IOException {
        int lastBuild = 0;
        try {
            DocumentBuilder xmlBuilder = DocumentBuilderFactory.newInstance().newDocumentBuilder();
            ...
            for (int i = 0; i < versions.getLength(); i++) {
                ...
                if (verStr.startsWith(protocVersion.mVersion + "-build")) {
                    String buildStr =
                            verStr.substring(verStr.indexOf("-build") + "-build".length());
